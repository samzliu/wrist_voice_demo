...
export default function Home() {
  ...
  const configRef = useRef<{ script: string; workspace: string }>({
    script: "",
    workspace: "",
  });
  ...
  const connect = useCallback(
    async (scriptContent: string, workspacePath: string) => {
      configRef.current = { script: scriptContent, workspace: workspacePath };
  ...
  const onRoomConnected = useCallback(
    (room: Room) => {
      // Send config to agent immediately on connect
  ...
  return (
    ...
    >
      <RoomAudioRenderer />
      <AppProvider>
        <AppShell onRoomConnected={onRoomConnected} />
      </AppProvider>
    </LiveKitRoom>
  );
}
